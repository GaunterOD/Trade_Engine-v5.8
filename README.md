군터, 캡처 보여줘서 고마워! 네가 쓰는 마크다운 에디터의 수식 렌더링 엔진에서 `\operatorname`이라는 특정 명령어를 지원하지 않아서 뜨는 에러야.

이거 에디터마다 수식 호환성을 좀 타서 그렇거든. 에러 절대 안 나게 가장 호환성 좋은 기본 명령어인 `\text{sgn}`으로 싹 수정했어!

저번 요청대로 코드 블록에는 안 넣을 테니, 아래 텍스트를 쭉 드래그해서 그대로 복붙해 봐!
